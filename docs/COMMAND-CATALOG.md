...
Five user-invoked skills (`disable-model-invocation: true`), each with arguments, defaults and a safety note. Invoke as `/adlc:<name>` once the plugin is installed. Status: 1 of 5 implemented.
...
| 2 | `/adlc:plan` | `$0` env; `--layer foundation\|app` | `dev`, `foundation` | plan only; a human applies with an approval token | stub | `/adlc:plan dev --layer foundation` |
