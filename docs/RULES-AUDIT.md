...
| Permissions | `ask` on `git commit`/`git push`; 55 accumulated `allow` entries in `settings.local.json` | allow-list is machine-local; adlc repos ship their own read-only allow-list in `.claude/settings.json` |
...
### 1b. `integranz/adlc` (plugin + marketplace)
| Layer | File | Content |
|---|---|---|
...
| 8 | Plugin repo had no `.claude/rules/` | adlc | `precedence.md` added so both repos in scope carry the same precedence statement | done |
| 9 | User `settings.local.json` allow-list (55 entries incl. Ontellus/ADO commands) applies inside adlc sessions | user scope | Harmless (allow-only, read-mostly); adlc repos add their own explicit allow-list; no `deny` rules exist anywhere, so nothing can conflict | accepted |
...
Skill name conflicts follow Claude Code's documented order (enterprise → personal → project); plugin skills are namespaced (`adlc:<name>`) and therefore never collide.
