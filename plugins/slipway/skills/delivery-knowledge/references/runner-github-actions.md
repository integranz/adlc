...
| `test` | PR + default branch | each app's `test_command` from `.slipway/config.yaml` | setup steps only for stacks present (`has_dotnet`, `has_node`) |
...
## Decisions baked in
- **No `workflow_run` handoff**: it cannot carry inputs. CD is a separate `workflow_dispatch` workflow that receives the tag explicitly (`/slipway:deploy`).
