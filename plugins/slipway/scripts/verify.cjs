#!/usr/bin/env node
// Deterministic post-deployment verification for slipway (cloud=azure, compute=aca, runner=github-actions).
...
(async () => {
  ...
  const evidencePath = val("--evidence", path.join(repo, ".slipway", "evidence", `${tag}.md`));
