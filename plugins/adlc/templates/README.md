...
Rendered into a target repository by `scripts/scaffold.cjs` according to `.adlc/config.yaml`.
...
| `common/adlc/*` | never rendered: option registry, config schema, example config used by the plugin itself | — |
...
Adding an option: set its `status` in `common/adlc/options.yaml`, add the enum value to `common/adlc/config.schema.json` and rebuild the validator (`node scripts/build-validator.cjs` in the repo root), then add `<dimension>/<option>/files/` and `skills/delivery-knowledge/references/<dimension>-<option>.md`.
