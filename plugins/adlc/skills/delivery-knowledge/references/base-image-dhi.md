...
- Tag policy for adlc: pin the major.minor from `build.*_version` (`8.0`, `22`, `1.29`); CI rebuilds pick up patch updates from the registry.
...
- **Labels**: `org.opencontainers.image.version`, `.revision`, `.source`, `.title` on every runtime stage so `/adlc:verify` can compare the running image to the tag and commit.
- **Ports**: everything listens on the app's `port` (8080 by default) as non-root; never bind < 1024.

## Local verification recipe (what `/adlc:dockerize` runs)
```
