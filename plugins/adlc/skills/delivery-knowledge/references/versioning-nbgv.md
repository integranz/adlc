...
- Images are tagged with `SemVer2`; the same value is stamped into the API (`-p:InformationalVersion`) and the web bundle (`VITE_APP_VERSION`) so `/adlc:verify` can compare the running version with the tag.
...
- Exercised on `integranz/adlc-demo` (first CI release on 2026-09-15).
