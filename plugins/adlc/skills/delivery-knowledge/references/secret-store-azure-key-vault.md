...
- **How apps receive secrets on Container Apps** (`infra/app`, day 7): `secret { name = "db-password" key_vault_secret_id = "<vault uri>/secrets/db-password" identity = <identity id> }` plus `env { name = "DB_PASSWORD" secret_name = "db-password" }`. Container Apps fetches the value with the identity and refreshes it periodically; nothing is stored in Terraform state.
