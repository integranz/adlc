...
|---|---|---|
| `.adlc/setup-azure.sh` (human, once) | resource group, state storage account + container, Entra app registration for CI/CD, RBAC for the CI/CD principal and the human | Chicken-and-egg (state before Terraform) and privileged operations that Terraform should not need |
...
## Naming rendered from `.adlc/config.yaml`
...
## Apply protocol
1. `/adlc:plan <env> --layer foundation` → `infra/foundation/tfplan.<env>` + summary.
