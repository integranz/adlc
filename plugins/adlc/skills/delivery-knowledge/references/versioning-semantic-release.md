...
- Source of truth: commit messages (Conventional Commits via `conventional-changelog-conventionalcommits@^8`; v10 of the preset is incompatible with the writer bundled in `@semantic-release/release-notes-generator`, learned on `integranz/adlc`). `feat:` → minor, `fix:` → patch, `!`/`BREAKING CHANGE:` → major; `docs:`/`chore:` → no release.
