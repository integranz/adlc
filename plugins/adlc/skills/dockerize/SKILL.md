---
name: dockerize
description: Build the hardened multi-stage container image for one app from the adlc templates, run it locally, and prove it answers its health endpoint with the expected version as a non-root user.
disable-model-invocation: true
allowed-tools: Read, Glob, Grep, Agent, Bash(node "${CLAUDE_PLUGIN_ROOT}/scripts/scaffold.cjs" *), Bash(node "${CLAUDE_PLUGIN_ROOT}/scripts/validate-config.cjs" *), Bash(nbgv get-version *), Bash(git rev-parse *), Bash(docker build *), Bash(docker run *), Bash(docker rm *), Bash(docker stop *), Bash(docker image inspect *), Bash(docker inspect *), Bash(docker top *), Bash(docker logs *), Bash(docker images *), Bash(docker compose *), Bash(curl *), Bash(sleep *)
---

# /adlc:dockerize — image for one app

Arguments: `$0` = app path or app name from `.adlc/config.yaml` (e.g. `apps/api` or `api`). Flags: `--no-run` (build only), `--compose` (after the single-app check, run every app with `compose.yaml` and test the frontend → API proxy), `--force` (re-render the Dockerfile even if it was edited).

## Preconditions
1. `.adlc/config.yaml` exists and validates: `node "${CLAUDE_PLUGIN_ROOT}/scripts/validate-config.cjs" .adlc/config.yaml`. Otherwise stop and point to `/adlc:bootstrap`.
...
`node "${CLAUDE_PLUGIN_ROOT}/scripts/scaffold.cjs" --repo . --app <name>` (add `--force` only if asked). This writes `<app path>/Dockerfile`, `.dockerignore`, and for frontends `nginx.conf` + `nginx.local.conf`. If the scaffold reports a config error (for example no `.csproj` detected), stop and tell the user which `build.*` key to set in `.adlc/config.yaml`. Never hand-edit the Dockerfile: fix the template or the config.

## Step 2 — Build (execute sub-agent)
Delegate to `execute` with this acceptance command, substituting the values:
```
VERSION=$(nbgv get-version -v SemVer2 2>/dev/null || echo 0.0.0-local); COMMIT=$(git rev-parse --short HEAD)
docker build --build-arg VERSION=$VERSION --build-arg COMMIT=$COMMIT -t <project>/<app>:$VERSION <app path>
```
...
| Claim | Check |
|---|---|
| Container starts and stays up | `docker run -d --rm -p <port>:<port> --name adlc-<app> <image>` then `docker inspect -f '{{.State.Running}}'` after 3 s |
...
Always stop the container afterwards (`docker stop adlc-<app>`). Frontends with upstreams cannot reach the API standalone; mark the proxy claim UNVERIFIABLE unless `--compose` is used.
...
## Output
```
## adlc dockerize: <project>/<app> <version>
...
- Do not edit the generated Dockerfile, nginx config or `.dockerignore` by hand; change the template in the plugin or `build.*` in the config.
