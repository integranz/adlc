...
const V = path.join(__dirname, "verify.cjs"); const EXAMPLE = path.join(__dirname, "..", "templates", "common", "adlc", "config.example.yaml");
function repo() { const r = fs.mkdtempSync(path.join(os.tmpdir(), "adlc-verify-")); execFileSync("git", ["init", "-q", "-b", "main", r]); fs.mkdirSync(path.join(r, ".adlc")); fs.writeFileSync(path.join(r, ".adlc", "config.yaml"), yaml.dump(yaml.load(fs.readFileSync(EXAMPLE, "utf8")))); return r; }
test("verify.cjs refuses bad arguments before touching anything", () => {
  ...
  assert.ok(!fs.existsSync(path.join(r, ".adlc", "evidence", "latest.md")), "must not write evidence for invalid input");
});
test("verify.cjs --no-write leaves the repo untouched and reports per claim", () => {
  ...
  assert.ok(!fs.existsSync(path.join(r, ".adlc", "evidence", "0.1.0.md")));
});
