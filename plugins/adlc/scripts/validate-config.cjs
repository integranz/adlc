#!/usr/bin/env node
// Validates a target repo's .adlc/config.yaml: schema + option statuses + cross-dimension constraints. Exit 1 on errors.
"use strict";
const path = require("node:path");
const { loadConfig } = require("./lib/config.cjs");
const file = path.resolve(process.argv[2] || ".adlc/config.yaml");
let res;
