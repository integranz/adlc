...
ttl="${ADLC_APPROVAL_TTL:-600}"; exp=$(( $(date +%s) + ttl ))
mkdir -p "$root/.adlc/approvals"
printf '%s\n%s\n%s\n' "$sha" "$exp" "$(basename "$plan") approved by $(id -un) at $(date -u +%FT%TZ)" > "$root/.adlc/approvals/$sha"
...
case "$(git -C "$root" check-ignore -q .adlc/approvals/x 2>/dev/null; echo $?)" in 0) ;; *) echo "note: add '.adlc/approvals/' to .gitignore (approval tokens must not be committed)";; esac
